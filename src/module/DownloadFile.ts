...
const downloadFile: (file: FileInfo, onDownloadStart: (downloadId: number) => void) => Promise<void> = async (file, onDownloadStart) => {
  const fileName = decodeURI(file.url.match(".+/(.+?)([?#;].*)?$")[1]);
  const filePath = PathModule.join("Manaba", file.courseName.replace("/", "-"), file.contentName.replace("/", "-"), fileName.replace("/", "-"));
  const downloadId = await chrome.downloads.download({ url: file.url, filename: filePath, saveAs: false });
